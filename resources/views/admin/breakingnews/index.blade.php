	...
	<div class="col-md-12">
		...
		<div class="card">
			<div class="card-header card-header-primary card-header-icon">
				...
				<h4 class="card-title">Mob Home PopUp <a href="{{ url('admin/pool/create') }}" class="btn btn-primary btn-sm pull-right">Add New</a></h4>
			</div>
			<div class="card-body">
					...
					<table class="table table-striped table-no-bordered table-hover datatables" cellspacing="0" width="100%" style="width:100%">
						...
						<tbody>
							...
							<tr>
								...
								<td class="text-primary">{{ $row->start_at }}</td>
								<td class="text-primary">{{ $row->end_at }}</td>
								<td>
									@if($row->end_at < date('Y-m-d H:i:s') && $row->end_at!='')
								...
								<td class="text-right">
									<a href="{{ url('admin/breakingnews/'.$row->id.'/edit') }}" class="btn btn-link btn-warning btn-just-icon edit"><i class="material-icons">edit_note</i></a>
									<form class="pull-right" method="POST" action="{{ route('breakingnews.destroy', $row->id) }}">
										@csrf @method('DELETE')
