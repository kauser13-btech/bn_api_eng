...
<form class="form-horizontal" action="{{ route('gallery.categoryupdate') }}" method="POST">
			@csrf

	<input type="hidden" name="row-id" value="{{ $cat->id }}">
							...
							<div class="col-sm-8">
								<div class="form-group">
									<select id="Gtype" class="selectpicker" data-size="7" data-style="select-with-transition" name="type">
										<option @if($cat->type=='photo') selected @endif value="photo">Photo</option>
										<option @if($cat->type=='video') selected @endif value="video">Video</option>
									</select>
								</div>
							</div>
							<label class="col-sm-2 label-on-right"><code>required</code></label>
								...
								<div class="form-group">
									<div class="form-check">
										<label class="form-check-label">
											<input class="form-check-input" type="radio" name="g_status" value="1" @if($cat->g_status==1) checked @endif> Active
											<span class="circle"><span class="check"></span></span>
										</label>
									</div>
									<div class="form-check">
										<label class="form-check-label">
											<input class="form-check-input" type="radio" name="g_status" value="0" @if($cat->g_status==0) checked @endif> Inactive
											<span class="circle"><span class="check"></span></span>
										</label>
									</div>
								</div>
