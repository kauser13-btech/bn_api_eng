...
<body>
  ...
  @if (Request::get('type') == 'Images' && Request::get('CKEditor') == 'editor')
    <script>
      @if (Auth::user()->folder_location == '')
        goTo("/shares/{{ Auth::user()->type == 'print' ? Auth::user()->type : 'online' }}/{{ date('Y/m/d') }}/my1{{ Auth::user()->id }}");
      @else
        goTo("/shares/{{ Auth::user()->folder_location }}");
      @endif
    </script> @endif
